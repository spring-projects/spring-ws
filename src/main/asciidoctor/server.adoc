...
public class AnnotationOrderEndpoint {
  ...
  @PayloadRoot(localPart = "order", namespace = "https://samples")                              // <5>
  public void order(@RequestPayload Element orderElement) {                                    // <3>
    Order order = createOrder(orderElement);
    orderService.createOrder(order);
  }

  @PayloadRoot(localPart = "orderRequest", namespace = "https://samples")                       // <5>
  @ResponsePayload
...
For more information about endpoint methods, refer to <<server-atEndpoint-methods>>.
<5> The two handling methods of this endpoint are marked with `@PayloadRoot`, indicating what sort of request messages can be handled by the method: the `getOrder` method will be invoked for requests with a `orderRequest` local name and a `https://samples` namespace URI; the order method for requests with a `order` local name.
...
[source,java]
----
@PayloadRoot(localPart = "order", namespace = "https://samples")
...
public class AnnotationOrderEndpoint {
  ...
  @PayloadRoot(localPart = "orderRequest", namespace = "https://samples")
  *@Namespace(prefix = "s", uri="https://samples")*
...
Since we use the prefix `s` in our XPath expression, we must bind it to the `https://samples` namespace. This is accomplished with the `@Namespace` annotation. Alternatively, we could have placed this annotation on the type-level to use the same namespace mapping for all handler methods, or even the package-level (in `package-info.java`) to use it for multiple endpoints.
...
<SOAP-ENV:Envelope xmlns:SOAP-ENV="http://www.w3.org/2003/05/soap-envelope"
  ...
  <SOAP-ENV::Header>
    ...
    <wsa:To S:mustUnderstand="true">https://example/com/fabrikam</wsa:To>
...
public class AnnotationOrderEndpoint {
    ...
    @Action("https://samples/RequestOrder")
    public Order getOrder(OrderRequest orderRequest) {
        return orderService.getOrder(orderRequest.getId());
    }

    @Action("https://samples/CreateOrder")
...
The mapping above routes requests which have a WS-Addressing `Action` of `https://samples/RequestOrder` to the `getOrder` method. Requests with `https://samples/CreateOrder` will be routed to the `order` method..
