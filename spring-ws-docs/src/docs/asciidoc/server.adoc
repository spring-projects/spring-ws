...
[[server-http-sender-validation]]
==== HTTP message senders and out-of-band validation

For WS-Addressing `ReplyTo` or `FaultTo` (see <<server-endpoint-out-of-band-replies>>), HTTP-based message senders validate the peer-supplied URL before opening a connection:

* The URL must have a real host (not empty or obviously local-only forms).
* Literal IP addresses that are loopback, link-local, or "any" are rejected without DNS lookups, which blocks many metadata-style and lateral-movement URLs.
* Ordinary host names are not resolved by default to avoid a performance penalty, so malicious hostnames resolving to internal IPs are not blocked by the default IP-level checks. To secure out-of-band replies without the DNS resolution latency, use a `DestinationPolicy` to configure a strict allowlist. Alternatively, DNS resolution can be enabled by setting `allowDnsResolution` to `true` to apply IP-level checks to the resolved addresses.
* RFC&nbsp;1918-style private *IPv4 literals* are rejected by default; they can be enabled by setting `allowSiteLocalIpv4` to `true` while still composing with the built-in checks.
* HTTP clients may follow redirects depending on the stack (`HttpURLConnection` vs Apache HttpComponents and its redirect strategy). Treat redirects as part of your threat model for out-of-band POSTs, or configure the client not to follow them if your environment requires it.

Register a `DestinationPolicy` on the sender for allow-lists, DNS-backed checks, or other organization rules; it composes with these defaults.
...
[[server-jms-reply-destinations]]
==== Reply destinations

The JMS transport follows the JMS request/reply idiom: the response is sent to the `JMSReplyTo` destination of the request, correlated with its `JMSCorrelationID`, and inherits the request's delivery mode and priority.
A request that carries no `JMSReplyTo` gets no response.

It means the sender of a request chooses where its response is delivered, so the inbound destination should be treated as a trust boundary.
Restrict who may produce onto the inbound destination, and use your broker's per-destination authorization to constrain which destinations the application's own session may produce to.
Spring-WS does not screen `JMSReplyTo`, because on this transport it is the reply channel itself rather than an address supplied inside the message.

NOTE: This is different from the out-of-band replies described below.
A `wsa:ReplyTo` address travels inside the SOAP envelope, is acted on after the request has been processed, and can direct a reply onto a transport other than the one the request arrived on.
Those addresses *are* screened.

[[server-jms-sender-validation]]
==== Out-of-band JMS replies

When a JMS message sender is used for <<server-endpoint-out-of-band-replies,out-of-band replies>>, destination and reply names are screened for patterns that often indicate unsafe indirection (embedded LDAP- or JNDI-style fragments and similar). If your environment uses names that legitimately contain such substrings, attach a `DestinationPolicy` to the sender so you can express allow-lists or resolver-specific exceptions while still composing with the defaults.

[[server-mail-transport]]
=== Email Transport
...
[[server-mail-sender-validation]]
==== Out-of-band mail replies

When a mail message sender is used for <<server-endpoint-out-of-band-replies,out-of-band replies>>, the mailbox must be well-formed and local-only mailbox hosts are rejected by default. Use a `DestinationPolicy` for domain allow-lists or stricter rules.
...
If you configure HTTP-based `WebServiceMessageSender` beans for WS-Addressing out-of-band replies, they follow the same validation rules as in servlet-based deployments; see <<server-http-sender-validation>>.
...
[[server-xmpp-sender-validation]]
==== Out-of-band XMPP replies

When an XMPP message sender is used for <<server-endpoint-out-of-band-replies,out-of-band replies>>, the Jabber ID's domain must match the service domain of the configured connection. Use a `DestinationPolicy` if you need allow-lists or multi-tenant rules.
...
| SOAP header (unmarshalled)
| Any type (or `List` thereof) supported by a Spring OXM {spring-framework-docs}/data-access/oxm.html#oxm-marshaller-unmarshaller[`Unmarshaller`], when used in combination with the `@SoapHeader` annotation.
| No
| Enabled when the `unmarshaller` attribute of `<sws:annotation-driven/>` is specified. See <<server-soap-header-unmarshalling>>.
...
[[server-soap-header-unmarshalling]]
==== Unmarshalling `@SoapHeader` Parameters

By default, a parameter annotated with `@SoapHeader` must be of type `SoapHeaderElement` (or a `List` of them, for headers that occur multiple times).
When an `unmarshaller` has been configured (through the `unmarshaller` attribute of `<sws:annotation-driven/>`), you can instead annotate a parameter of any type supported by that unmarshaller, in which case the content of the matching SOAP header is unmarshalled into the target type.
This saves you from having to manually retrieve the `Source` from the `SoapHeaderElement` and pass it to an unmarshaller yourself.

The following method is invoked with the content of the `{http://samples}security` SOAP header unmarshalled into a `Credentials` object:

====
[source,java]
----
public void handle(@RequestPayload OrderRequest orderRequest, @SoapHeader("{http://samples}security") Credentials credentials)
----
====

If no header with a matching qualified name is found, the parameter is resolved as `null`.
If the header occurs multiple times, you can declare the parameter as `List<Credentials>` instead, in which case every matching header is unmarshalled and added to the list.
...
[[server-endpoint-out-of-band-replies]]
=== Out-of-band Replies

When WS-Addressing signals a non-anonymous reply or fault path, the response cannot ride the original inbound transport and Spring-WS must open a separate connection. Configure `messageSenders` on your endpoint mapping with the same `WebServiceMessageSender` implementations you would use on a client `WebServiceTemplate`; see <<client-transports>> for transport overview and wiring.

Spring-WS tries each configured sender in order; the first one that supports the `wsa:ReplyTo` or `wsa:FaultTo` URI sends the message. Because those addresses are chosen by the remote party, senders apply stricter validation before opening a connection than they would when your application supplied the destination. If the built-in rules are too strict for your environment (for example names that legitimately contain unusual substrings) or too loose (for example you need explicit allow-lists), register a `DestinationPolicy` on the sender bean so your rules compose with the defaults.

IMPORTANT: When WS-Addressing out-of-band senders are configured, security interceptors must run before the WS-Addressing interceptor to prevent unauthenticated requests from triggering fault delivery to an attacker-supplied `wsa:FaultTo` address. Spring-WS guarantees this by ordering WS-Security interceptors (typically `Wss4jSecurityInterceptor`) before the addressing interceptor regardless of how they are registered. See `AbstractWsSecurityInterceptor.DEFAULT_ORDER` and `AddressingEndpointInterceptor.DEFAULT_ORDER` for the concrete order values.

Default checks for peer-supplied reply and fault addresses are documented per transport:

* <<server-http-sender-validation,HTTP message senders>>
* <<server-jms-sender-validation,JMS message senders>>
* <<server-mail-sender-validation,Mail message senders>>
* <<server-xmpp-sender-validation,XMPP message senders>>
...
Additionally, there is the `Wss4jSecurityInterceptor`, which is described in <<security>>.
...
WARNING: `SimpleSoapExceptionResolver` is one of the resolvers that the `MessageDispatcher` uses when the application context declares no `EndpointExceptionResolver` bean, so this is what an endpoint does out of the box.
The fault string is the message of the exception, or its `toString()` representation if it has none, and the caller of the endpoint sees it.
An exception raised by your own code, or by a library that it uses, can carry internal detail such as a file path, a SQL statement, or a host name.
<<server-endpoint-exception-resolver-simple,Subclassing this resolver>> does not change that, as it only lets you customize the detail of the fault, not its fault string.
To decide what the fault string says, declare a <<server-endpoint-exception-resolver-mapping,`SoapFaultMappingExceptionResolver`>> instead.

[[server-endpoint-exception-resolver-simple]]
=== `SimpleSoapExceptionResolver`

Where the fault string of the exception message is acceptable, this resolver needs no configuration at all, as the `MessageDispatcher` falls back to it.
Subclass it to add a detail element to every fault it produces, by overriding `customizeFault`:
...
The fault string is set on the fault before `customizeFault` is called, so this is the place to describe the failure further, not to replace what it already says.

[[server-endpoint-exception-resolver-mapping]]
...
The default fault handles every exception that is not mapped, so giving it a fault string of its own, as the example above does, is what keeps the message of an exception you did not anticipate from reaching the caller.
Such an exception is otherwise only recorded if the `warnLogCategory` property is set, which is worth doing once the fault no longer carries its detail.
